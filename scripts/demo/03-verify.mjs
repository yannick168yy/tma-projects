...
// 拿去扫已脱敏字段只会得到一堆误报。
// 前后的边界约束不能省：没有它时 "profitPhp": -70.09123456789 里的
// 09123456789 会被当成菲律宾手机号报出来，浮点金额几乎行行命中。
// 误报比漏报更糟 —— 一旦习惯性忽略告警，真的泄露也会被划过去。
// (?<![0-9.]) 排除紧跟在数字或小数点后面的串，(?![0-9]) 排除后面还有数字的。
const PII_PATTERNS = [
  { name: '手机号', re: "(?<![0-9.])(\\+?63|0)9[0-9]{9}(?![0-9])|(?<![0-9.])(\\+?62|0)8[0-9]{8,11}(?![0-9])" },
  { name: '身份证/NIK', re: "(?<![0-9.])[0-9]{16}(?![0-9])" },
]
