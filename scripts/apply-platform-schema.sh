...
for f in "$ROOT"/infra/database/platform/[0-9][0-9][0-9]_*.sql; do
  [[ -f "$f" ]] || continue
  ver="$(basename "$f" .sql)"
  done_cnt="$(mysql_cli "$PLATFORM_DB" -sN -e "SELECT COUNT(*) FROM schema_migrations WHERE version='$ver'" | tr -d ' ')"
  if [[ "${done_cnt:-0}" -gt 0 ]]; then SKIP=$((SKIP+1)); continue; fi
  mysql_cli --default-character-set=utf8mb4 "$PLATFORM_DB" < "$f"
