...
--   2% 档(≤9款):  RTP≤0.95(hold≥5%) → 洗码2% 占净GGR ~43%
--   1.5%档(≤30款): RTP≤0.96(hold≥4%) → 占净 ~41%
...
ORDER BY weight DESC, game_id LIMIT 9;

-- 1.5% 档：RTP≤0.96 热度前 30（排除已入 2% 档）
INSERT INTO bg_rebate_featured_game (game_uuid, tier, sort_order, enabled)
...
ORDER BY weight DESC, game_id LIMIT 30;
