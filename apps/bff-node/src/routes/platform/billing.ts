...
export function createBillingRouter(): Router {
  ...
  router.post('/tenants/:id/daily/recompute', write, async (ctx) => {
    ...
    const rows = await runWithTenant(tenant, () => snapshotTenantDay(ctx.state.env, date))
    await writeAudit(ctx.state.platformAdmin?.adminId ?? null, ctx.ip, 'billing.daily.recompute', id, { date, rows })
    ok(ctx, { date, rows })
  })
