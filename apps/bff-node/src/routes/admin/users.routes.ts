...
router.get('/:id', async (ctx) => {
  ...
  ok(ctx, {
    ...
    balanceCurrency,
    growth,
    depositTotal: depositTotals.get(ctx.params.id)?.usdt ?? 0,
    depositByCurrency: depositTotals.get(ctx.params.id)?.byCurrency ?? [],
    withdrawTotal: withdrawTotals.get(ctx.params.id)?.usdt ?? 0,
...
const SUPPORTED_CURRENCIES = ['PHP', 'IDR', 'INR', 'USDT', 'USDC', 'TRX_TESTNET']
