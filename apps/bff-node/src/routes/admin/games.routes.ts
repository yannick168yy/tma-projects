...
router.get('/homepage-layout', async (ctx) => {
  try {
    ...
    if (!['PHP', 'IDR', 'INR', 'USDT'].includes(currency)) { fail(ctx, 400, 'currency 必须为 PHP、IDR、INR 或 USDT'); return }
...
// body: { currency: 'PHP'|'IDR'|'INR'|'USDT', hidden: boolean }
router.put('/homepage-sections/:sectionKey/visibility', async (ctx) => {
  try {
    ...
    if (!body.currency || !['PHP', 'IDR', 'INR', 'USDT'].includes(body.currency)) { fail(ctx, 400, 'currency 必须为 PHP、IDR、INR 或 USDT'); return }
...
// body: { currency: 'PHP'|'IDR'|'INR'|'USDT' }
router.post('/homepage-sections/:sectionKey/freeze', async (ctx) => {
  try {
    ...
    if (!currency || !['PHP', 'IDR', 'INR', 'USDT'].includes(currency)) { fail(ctx, 400, 'currency 必须为 PHP、IDR、INR 或 USDT'); return }
...
// 解冻：删除该板块该币种的冻结名单，回到算法。query: ?currency=PHP|IDR|INR|USDT
