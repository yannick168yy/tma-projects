...
export const RULE_META: Record<string, { name: string; desc: string }> = {
  ...
  bonus_bet_abuse:          { name: '上游彩金异常', desc: '一段时间内，玩家领取的“平台活动奖金”（如奖池大奖、锦标赛、抽奖等由游戏商搞活动派发的奖金）总额超过设定金额、或笔数超过设定次数，疑似专门薅活动羊毛，转人工复核。只算这类活动奖金，不含玩家在老虎机里自己转出来的免费旋转中奖，避免误伤正常玩老虎机的人。（注：目前平台还没开这类活动，此规则暂时不会触发，等上线奖池/锦标赛类玩法再按实际情况调金额和次数。）' },
  feature_bonus_ratio:      { name: '老虎机彩金倍数', desc: '玩家一段时间内在老虎机“免费旋转/彩金玩法”里赢到的钱，除以他历史累计的真实充值金额，倍数达到设定值就转人工复核。例如只充了 500、却靠免费旋转赢了 6000，就是 12 倍。用来兜底抓“小额充值、靠老虎机彩金爆量后马上提现套现”的人。玩家从没真实充值过则跳过（交给其它规则处理）。' },
