...
export function startWin568GameSyncCron(app: FastifyInstance): void {
  ...
  const run = () => void runForProviderTenants(app, 'win568-game-sync', 'win568', () => syncWin568Games(app))
...
async function syncWin568Games(app: FastifyInstance): Promise<void> {
  try {
    ...
    const result = await new Win568Client(companyKey, await getWin568ServerId(app)).getGameList({ GpId: 1, IsGetAll: true })
