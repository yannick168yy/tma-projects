...
import { Alert, Button, Card, Descriptions, Form, Input, InputNumber, Modal, Popconfirm, Select, Space, Switch, Table, Tag, Tooltip, Typography, message } from 'antd'
import {
  addTenantDomain, deleteTenantI18n, getTenantBrand, getTenantDetail, getTenantFeatures, impersonateTenant, setDomainAcme,
  saveTenantProvider, syncTenantProvider, saveTenantChannel, deleteTenantChannel, syncTenantChannels,
  ...
  getTenantApps, saveTenantApp, deleteTenantApp,
  type I18nCatalogEntry, type I18nOverrideRow, type TenantAppBuild,
  type TenantBrandResponse, type TenantDetail as Detail, type TenantFeatures,
  type TenantProvider, type TenantChannel,
} from '../api'
...
export default function TenantDetail() {
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size={16}>
      ...
      <IntegrationCard tenantId={d.id} detail={d} onChanged={load} />
    </Space>
  )
}
...
function AppBuildCard({ tenantId }: { tenantId: number }) {
...
function IntegrationCard({ tenantId, detail, onChanged }: {
  tenantId: number; detail: Detail; onChanged: () => Promise<void> | void
}) {
          ...
          <Space>
            <Button size="small" disabled={!keyReady}
              onClick={() => setChannelEdit({
                ...
                enabled: true, sortOrder: 100, credential: '',
              })}>+ 分配通道</Button>
