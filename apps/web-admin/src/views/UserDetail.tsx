...
import {
  ...
  Spin, Tabs, Table, Typography, Divider, message, Progress, Modal, Alert,
} from 'antd'
import { getUserDetail, updateUserStatus, updateUserLabel, adjustBalance, updateUserProfile, SUPPORTED_CURRENCIES, getUserTurnover, adjustTurnoverRequirement, type TurnoverRequirement } from '../api'
...
export default function UserDetail() {
  ...
  async function loadTurnover() {
    ...
    try { setTurnover(await getUserTurnover(id)) }
  ...
  async function doAdjustTurnover(action: 'adjust' | 'cancel') {
    ...
    try {
      await adjustTurnoverRequirement(id, adjustModal.req.id, action, action === 'adjust' ? adjustValue : undefined, adjustReason || undefined)
                    ...
                    { key: 'bets', label: `游戏记录 (${detail.betOrders.length})`, children: <Table columns={betCols} dataSource={detail.betOrders} rowKey="id" pagination={false} size="small" /> },
                    {
                      ...
                      children: (
                        <Spin spinning={turnoverLoading}>
                          {turnover && (
                            <div>
                              <Alert
                                ...
                                message={turnover.canWithdraw
                                  ? '流水要求已全部完成，可提款'
                                  : `流水未完成，还需完成 ${turnover.totalRemaining.toFixed(4)}`}
                                showIcon
                              />
                              <Table
                                columns={turnoverCols}
                                dataSource={turnover.requirements}
                                rowKey="id"
                                pagination={false}
                                size="small"
                              />
                          ...
                          {!turnover && !turnoverLoading && (
                            <Typography.Text type="secondary">点击「流水记录」Tab 加载数据</Typography.Text>
                          )}
                        </Spin>
                      ),
                    },
            ...
            <Space direction="vertical" style={{ width: '100%' }}>
              <div>
                ...
                <InputNumber
                  ...
                  min={0}
                  max={adjustModal.req.requiredAmount}
                  precision={4}
                  step={1}
                  style={{ width: '100%' }}
                />
