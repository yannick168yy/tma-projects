...
export interface Preview { revision: string; changed: number; missing: number; unavailable: number; blocking: number; unmapped: number; unmappedItems: { uuid: string; name: string }[]; rows: { id: number; currency: string; name: string; enabled: boolean; before: string | null; after: string | null; changed: boolean; level: string; issue: string | null; currencies: string[]; aliases: string[] }[] }
export type Change = ({ kind: 'provider' } & Omit<Provider, 'id'> & { id?: number })
  ...
  | { kind: 'rule'; scope: Rule['scope']; targetId: number; currency: RouteCurrency; aggregator: Aggregator | null }
