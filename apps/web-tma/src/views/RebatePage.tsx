  ...
  return (
    <section className="mx-4 mt-6">
      ...
      <div className="mt-3 space-y-2">
        {steps.map((step, i) => (
          <div key={step.title} className="flex items-center gap-3 rounded-xl bg-[#1a0e33]/75 border border-violet-400/20 px-3 py-2.5">
            <span className="flex h-[26px] w-[26px] flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-b from-[#ffd257] to-[#e89b12] text-[13px] font-black text-[#3a2a0d]">
              {i + 1}
            </span>
          ...
          return (
            <div key={faq.q} className="rounded-xl bg-[#1a0e33]/75 border border-violet-400/20 overflow-hidden">
              ...
              >
                ...
                <svg viewBox="0 0 16 16" className={`w-3.5 h-3.5 flex-shrink-0 text-[#ffc93d] transition-transform ${open ? 'rotate-180' : ''}`} fill="none">
      ...
      <ul className="mt-3 space-y-1.5 rounded-xl bg-[#1a0e33]/75 border border-violet-400/20 px-3.5 py-3">
        {notes.map((note) => (
          <li key={note} className="flex gap-2 text-[11px] leading-snug text-violet-200/60">
            <span className="mt-[5px] h-[4px] w-[4px] flex-shrink-0 rounded-full bg-[#ffc93d]/80" />
            {note}
          </li>
        ))}
      </ul>
      ...
      <button
        type="button"
        onClick={onGoBet}
        className="rb-shine mt-6 w-full bg-gradient-to-b from-[#ffd257] to-[#e89b12] text-[#3a2a0d] font-black text-base rounded-full py-3.5 shadow-[0_4px_18px_rgba(232,155,18,0.4)] active:opacity-80 transition"
      >
        {t('cashback.goBet')}
      </button>
    </section>
            ...
            {claimableBreakdown.map((item) => (
              <div key={item.gameCategory} className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5 text-violet-100/70">
                  ...
                  <span className="text-[10px] text-[#ffc93d]/80">{item.ratePct}%</span>
                </span>
                <span className="font-semibold text-[#ffc93d]">+{amtStr(currency, item.rebateAmount)}</span>
              </div>
            ))}
          </div>
        )}
                  ...
                  <div key={tier} className="rounded-2xl bg-[#1a0e33]/75 border border-violet-400/25 overflow-hidden shadow-[0_0_18px_rgba(139,92,246,0.1)]">
                    <div className="flex items-center gap-3 p-3">
                      ...
                      <button
                        ...
                        className="rb-shine flex-shrink-0 flex items-center gap-1.5 bg-gradient-to-r from-[#ffd257] to-[#e89b12] text-[#3a2a0d] rounded-full pl-4 pr-1.5 py-1.5 active:opacity-80 transition-opacity shadow-[0_2px_10px_rgba(232,155,18,0.3)]"
                      >
                        <span className="font-bold text-xs">{t('cashback.viewBtn')}</span>
                        <span className="bg-[#3a2a0d]/30 text-[#fff6dd] text-[11px] font-bold rounded-full min-w-5 h-5 px-1.5 flex items-center justify-center">
                          {games.length}
                        </span>
                      </button>
                    </div>
        ...
        {levelCards.length > 0 && (
          <div className="rb-rise mt-5" style={{ animationDelay: '270ms' }}>
            <div className="flex items-center justify-between mb-3 mx-4">
              ...
              {token && progress && (
                <span className="bg-gradient-to-r from-[#ffd257] to-[#e89b12] text-[#3a2a0d] font-black text-xs rounded-full px-3 py-1">
            ...
            {token && progress && (
              <div className="mx-4 mb-3 bg-[#1a0e33]/75 rounded-2xl border border-violet-400/25 px-4 py-3">
                ...
                <div className="h-2 rounded-full bg-black/30 overflow-hidden mt-2">
                  <div
                    className="rb-progress-fill h-full bg-gradient-to-r from-[#ffe082] to-[#f2b535] rounded-full transition-all"
                    style={{ width: `${progressPct}%` }}
                  />
                </div>
                <p className="text-[11px] text-[#ffc93d]/80 mt-1.5">
            ...
            >
              {levelCards.map((lc) => {
                ...
                const cardCls = isMax
                  ? 'border-[#f2b535]/60 bg-gradient-to-br from-[#2c1552]/90 via-[#1c0f38]/85 to-[#471b56]/50 shadow-lg shadow-amber-500/10'
                  : isCurrent
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`font-black text-xl font-display ${isMax ? 'text-[#ffc93d]' : 'text-violet-50'}`}>
                            {t('cashback.levelTag', { level: lc.level })}
                          </span>
                          {isCurrent && (
                            <span className="bg-[#f2b535] text-[#3a2a0d] text-[10px] font-black rounded-full px-2 py-0.5">
                      ...
                      {isMax && (
                        <span className="bg-gradient-to-r from-[#ffd257] to-[#e89b12] text-[#3a2a0d] text-[10px] font-black rounded-md px-2 py-1">MAX</span>
                      )}
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="flex-1 text-right leading-tight">
                              <span className="block text-[#ffc93d] font-bold text-sm">+{amtStr(currency, bonus)}</span>
                            ...
                            <span className={`w-10 text-right font-black text-sm ${isMax ? 'text-[#ffc93d]' : 'text-[#ffc93d]/90'}`}>{r.ratePct}%</span>
                          </button>
                        )
                      })}
                    </div>
                    ...
                    <div className={`mt-3 rounded-lg py-2 text-center text-xs font-black ${
                      isCurrent || (progress && lc.level < userLevel)
                        ...
                        : isMax
                          ? 'bg-gradient-to-r from-[#ffd257] to-[#e89b12] text-[#3a2a0d]'
