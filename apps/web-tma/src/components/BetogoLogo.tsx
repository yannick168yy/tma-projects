...
export default function BetogoLogo({ showBadge = false, badgeText = 'B' }: Props) {
  return (
    <div className="relative flex items-center justify-center" role="img" aria-label="Bet.Go.Win">
      <div className="flex flex-col leading-none" style={{ gap: '2.5px' }}>
        <div className="flex items-baseline">
          <span className="font-display text-[1.3rem] font-black leading-none text-white">BETO</span>
        ...
        <div className="flex items-center gap-px leading-none">
          <span className="text-[0.54rem] font-extrabold text-white/45">Bet.</span>
          <span className="text-[0.54rem] font-extrabold text-primary">Go.</span>
          <span className="text-[0.54rem] font-extrabold text-white/45">Win</span>
        </div>
      </div>
      {showBadge && (
