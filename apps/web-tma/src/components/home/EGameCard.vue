...
<template>
  <div
    class="flex-shrink-0 w-32 rounded-xl overflow-hidden relative h-28 cursor-pointer"
    @click="emit('tap')"
  >
    ...
    <div class="absolute bottom-0 inset-x-0 p-1.5">
      <p class="text-white font-bold text-[11px] leading-tight truncate">{{ game.name }}</p>
    </div>
  </div>
</template>
