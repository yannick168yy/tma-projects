...
import BuntingStrip from './BuntingStrip.vue'
...
function fireworkTopLine(deg: number, i: number) {
  const rad = degRad(deg)
  const r1 = 8
  const r2 = 24 + (i % 3) * 5
  return {
    x1: 182 + r1 * Math.cos(rad),
    y1: 18 + r1 * Math.sin(rad),
    x2: 182 + r2 * Math.cos(rad),
    y2: 18 + r2 * Math.sin(rad),
    stroke: fireworkTopStrokeColors[i % 4],
  }
}

function fireworkMidLine(deg: number, i: number) {
  const rad = degRad(deg)
  return {
    x1: 202 + 5 * Math.cos(rad),
    y1: 58 + 5 * Math.sin(rad),
    x2: 202 + 15 * Math.cos(rad),
    y2: 58 + 15 * Math.sin(rad),
    stroke: fireworkMidStrokeColors[i % 4],
  }
}

function sunburstLine(deg: number, i: number) {
  const rad = degRad(deg)
  const r1 = 8
  const r2 = i % 2 === 0 ? 22 : 15
  return {
    x1: 109 + r1 * Math.cos(rad),
    y1: 26 + r1 * Math.sin(rad),
    x2: 109 + r2 * Math.cos(rad),
    y2: 26 + r2 * Math.sin(rad),
    stroke: i % 2 === 0 ? '#FFB800' : '#f97316',
  }
}
</script>
...
<template>
  <div
    ...
    style="background: linear-gradient(155deg, #12003a 0%, #4a0a80 22%, #8c1a00 58%, #1a0800 100%); min-height: 240px"
  >
    <!-- Ambient glow layers -->
    <div
      ...
      style="
        background:
          radial-gradient(ellipse at 85% 90%, rgba(255, 150, 0, 0.32) 0%, transparent 50%),
          radial-gradient(ellipse at 10% 20%, rgba(168, 85, 247, 0.22) 0%, transparent 48%);
      "
    />
    ...
    >
      <path
        d="M0 8 Q54 22 108 8 Q162 -6 216 8 Q270 22 324 8 Q378 -6 430 8"
        stroke="#FFB800"
        stroke-width="0.8"
        fill="none"
        opacity="0.35"
      />
      <g v-for="(x, i) in lanternXs" :key="`lantern-${i}`">
        <ellipse
          :cx="x"
          :cy="22 + (i % 2 === 0 ? -4 : 4)"
          rx="8"
          ry="10"
          :fill="lanternColors[i % lanternColors.length]"
        />
        <ellipse
          :cx="x"
          :cy="22 + (i % 2 === 0 ? -4 : 4) - 3"
    ...
    <!-- RIGHT SIDE: Festive scene — float + dancers + fireworks -->
    <svg class="absolute right-0 bottom-0" width="218" height="238" viewBox="0 0 218 238" fill="none">
      <!-- Firework burst top-right -->
      <line
        v-for="(deg, i) in fireworkTopDegrees"
        :key="`fw-top-${i}`"
        :x1="fireworkTopLine(deg, i).x1"
        :y1="fireworkTopLine(deg, i).y1"
        :x2="fireworkTopLine(deg, i).x2"
        :y2="fireworkTopLine(deg, i).y2"
        :stroke="fireworkTopLine(deg, i).stroke"
        stroke-width="2"
      ...
      <!-- Small firework mid -->
      <line
        v-for="(deg, i) in fireworkMidDegrees"
        :key="`fw-mid-${i}`"
        :x1="fireworkMidLine(deg, i).x1"
        :y1="fireworkMidLine(deg, i).y1"
        :x2="fireworkMidLine(deg, i).x2"
        :y2="fireworkMidLine(deg, i).y2"
        :stroke="fireworkMidLine(deg, i).stroke"
        stroke-width="1.5"
        stroke-linecap="round"
        opacity="0.72"
      />
      ...
      <path d="M20 64 Q18 73 20 79" stroke="#ec4899" stroke-width="1.2" fill="none" opacity="0.55" />
      <circle cx="42" cy="40" r="11" fill="#FFB800" opacity="0.9" />
      <ellipse cx="39" cy="34" rx="3" ry="3.5" fill="rgba(255,255,255,0.2)" />
      <path d="M42 51 Q40 60 42 66" stroke="#FFB800" stroke-width="1.2" fill="none" opacity="0.55" />
      <circle cx="62" cy="48" r="10" fill="#a855f7" opacity="0.85" />
      <ellipse cx="59" cy="43" rx="3" ry="3.5" fill="rgba(255,255,255,0.2)" />
      <path d="M62 58 Q60 66 62 72" stroke="#a855f7" stroke-width="1.2" fill="none" opacity="0.55" />

      <!-- Sunburst crown at apex -->
      <line
        v-for="(deg, i) in sunburstDegrees"
        :key="`sunburst-${i}`"
        :x1="sunburstLine(deg, i).x1"
        :y1="sunburstLine(deg, i).y1"
        :x2="sunburstLine(deg, i).x2"
        :y2="sunburstLine(deg, i).y2"
        :stroke="sunburstLine(deg, i).stroke"
        stroke-width="2.5"
        opacity="0.92"
      />
      ...
      <path d="M66 62 Q88 30 109 22 Q130 30 152 62" stroke="#ec4899" stroke-width="4.5" fill="none" stroke-linecap="round" />
      <path
        d="M66 62 Q88 30 109 22 Q130 30 152 62"
        stroke="#FFB800"
        stroke-width="2"
        fill="none"
        stroke-linecap="round"
      ...
      />
      <polygon
        v-for="(p, i) in crownPoints"
        :key="`crown-${i}`"
        :points="`${p.x - 4},${p.y + 10} ${p.x},${p.y} ${p.x + 4},${p.y + 10}`"
        :fill="i % 2 === 0 ? '#FFB800' : '#ec4899'"
      />
      <circle cx="66" cy="62" r="9" fill="#ec4899" />
      <circle cx="66" cy="62" r="4.5" fill="#FFB800" />
      <circle cx="152" cy="62" r="9" fill="#ec4899" />
      <circle cx="152" cy="62" r="4.5" fill="#FFB800" />

      <!-- Left column -->
      <rect x="62" y="62" width="10" height="100" rx="3" fill="#d4870a" />
      <rect x="64" y="62" width="3" height="100" fill="rgba(255,228,80,0.28)" />
      <rect v-for="y in columnBandYs" :key="`col-l-${y}`" x="60" :y="y" width="14" height="5" rx="2" fill="#FFB800" />

      <!-- Right column -->
      <rect x="146" y="62" width="10" height="100" rx="3" fill="#d4870a" />
      <rect x="148" y="62" width="3" height="100" fill="rgba(255,228,80,0.28)" />
      <rect v-for="y in columnBandYs" :key="`col-r-${y}`" x="144" :y="y" width="14" height="5" rx="2" fill="#FFB800" />

      <!-- Garland swags between columns -->
      <path d="M72 68 Q90 96 109 80 Q128 96 146 68" stroke="#ec4899" stroke-width="2.5" fill="none" opacity="0.92" />
      <circle cx="109" cy="80" r="5.5" fill="#ec4899" />
      <circle cx="109" cy="80" r="2.5" fill="#FFB800" />
      <circle cx="90" cy="88" r="3" fill="#f97316" opacity="0.8" />
      <circle cx="128" cy="88" r="3" fill="#f97316" opacity="0.8" />

      <path d="M72 86 Q90 116 109 98 Q128 116 146 86" stroke="#34d399" stroke-width="2" fill="none" opacity="0.82" />
      <circle cx="109" cy="98" r="5" fill="#34d399" />
      <circle cx="109" cy="98" r="2.5" fill="#FFB800" />

      <path d="M72 106 Q90 134 109 116 Q128 134 146 106" stroke="#60a5fa" stroke-width="1.8" fill="none" opacity="0.72" />
      <circle cx="109" cy="116" r="4" fill="#60a5fa" />
      <circle cx="109" cy="116" r="2" fill="#fff" opacity="0.7" />
      ...
      <path
        d="M4 163 Q17 148 30 163 Q43 148 56 163 Q69 148 82 163 Q95 148 108 163 Q121 148 134 163 Q147 148 160 163 Q173 148 186 163 Q199 148 212 163"
        stroke="#ec4899"
        stroke-width="4"
        fill="none"
      />
      ...
      <text x="15" y="187" font-size="14" fill="#fff" opacity="0.95">★</text>
      <text x="60" y="187" font-size="14" fill="#FFB800">★</text>
      <text x="153" y="187" font-size="14" fill="#FFB800">★</text>
      <text x="194" y="187" font-size="14" fill="#fff">★</text>

      <!-- Tassels -->
      <line
        v-for="(x, i) in tasselXs"
        :key="`tassel-${i}`"
        :x1="x"
        y1="163"
        :x2="x"
        :y2="169 + (i % 3) * 3"
        :stroke="tasselStrokeColors[i % 3]"
        stroke-width="2"
        opacity="0.72"
      />
      ...
      <!-- Wheels -->
      <circle cx="42" cy="215" r="19" fill="#1a0040" stroke="#FFB800" stroke-width="3.5" />
      <circle cx="42" cy="215" r="9" fill="#FFB800" />
      <line x1="42" y1="196" x2="42" y2="234" stroke="#c07000" stroke-width="2.5" />
      <line x1="23" y1="215" x2="61" y2="215" stroke="#c07000" stroke-width="2.5" />
      <circle cx="176" cy="215" r="19" fill="#1a0040" stroke="#FFB800" stroke-width="3.5" />
      <circle cx="176" cy="215" r="9" fill="#FFB800" />
      <line x1="176" y1="196" x2="176" y2="234" stroke="#c07000" stroke-width="2.5" />
      <line x1="157" y1="215" x2="195" y2="215" stroke="#c07000" stroke-width="2.5" />
      ...
      <line x1="8" y1="167" x2="1" y2="155" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      <line x1="20" y1="167" x2="27" y2="155" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      ...
      <line x1="38" y1="166" x2="30" y2="154" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      <line x1="50" y1="166" x2="58" y2="154" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      ...
      <line x1="168" y1="166" x2="160" y2="154" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      <line x1="180" y1="166" x2="188" y2="154" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      ...
      <line x1="198" y1="167" x2="191" y2="155" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />
      <line x1="210" y1="167" x2="217" y2="155" stroke="#f5c5a3" stroke-width="3.5" stroke-linecap="round" />

      <!-- Confetti bits (inside festive SVG) -->
    ...
    <!-- Confetti left side -->
    <div
      v-for="(s, i) in confettiLeft"
      :key="`confetti-left-${i}`"
      class="absolute rounded-sm opacity-55 pointer-events-none"
      :style="{
        top: `${s.top}px`,
        left: `${s.left}px`,
        width: `${s.w}px`,
    ...
    <!-- Hero copy + optional slot (winners ticker, etc.) -->
    <div class="px-4 pt-12 pb-5 relative" style="max-width: 50%">
      <slot />
    </div>

    <!-- Bottom bunting -->
    <div style="transform: rotate(180deg)">
      <BuntingStrip :colors="buntingColors" />
    </div>
  </div>
</template>
