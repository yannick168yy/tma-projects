...
run run -d --name tma-core-node --network "$NET" --ip "$PEER_IP_CORE_NODE" --restart=always \
  ...
  -e WXGAME_BASE_URL="${WXGAME_BASE_URL:-https://test-openapi.cpigame.com}" \
  -e WXGAME_ACCESS_KEY_ID="${WXGAME_ACCESS_KEY_ID:-}" \
  -e WXGAME_ACCESS_KEY_SECRET="${WXGAME_ACCESS_KEY_SECRET:-}" \
  -e WXGAME_ALLOWED_IPS="${WXGAME_ALLOWED_IPS:-}" \
  -e WXGAME_DEFAULT_CURRENCY="${WXGAME_DEFAULT_CURRENCY:-PHP}" \
  ...
  -e HUITONE_MERCHANT_KEY="${HUITONE_MERCHANT_KEY:-}" \
...
run run -d --name "$BFF_NAME" --network "$NET" "${BFF_IP_ARGS[@]}" --restart=always \
  ...
  -e TELEGRAM_OIDC_REDIRECT_URI="${TELEGRAM_OIDC_REDIRECT_URI:-}" \
  ...
  -e HUITONE_BASE_URL="${HUITONE_BASE_URL:-https://api.huitone.ai}" \
  -e HUITONE_MERCHANT_ID="${HUITONE_MERCHANT_ID:-}" \
  -e HUITONE_MERCHANT_KEY="${HUITONE_MERCHANT_KEY:-}" \
  -e HUITONE_NOTIFY_URL="${HUITONE_NOTIFY_URL:-https://www.188facai.com/api/v1/callback/huitone}" \
