...
| 13 | **bff 连接池嵌套获取死锁（🔴生产验收 P2 实锤）**：`getSpinStatus` 持有池连接期间调 `getSpinConfig(env)` 再取第二条连接，池 10 条时 ≥10 并发 `/spin/status` 外层握满、内层无限等待（mysql2 默认无限排队）→ **整池死锁**，之后所有走鉴权（authMiddleware 查 MySQL）的请求全部挂死，只有 /health 活着，MySQL 侧连接全空闲无查询，唯一解=重启 bff。生产 P2 tasks 页 VUS=40 触发，rebate/vip/wallet-history 三页 100% 超时 60s | 上线后 10 用户同时开任务页即整站冻死 | 同类 4 处全修（3dbb478）：spin 抽 `loadSpinConfig(conn)`、checkin claim 配置读挪到取连接前、task 社媒审批发奖挪到 release 后、注册流 saveUser 挪到取外层连接前。**Backlog**：全库静态排查「持连接期间再打池」+ 评估 pool queueLimit 快速失败 | ✅已修，生产复测 40 并发 30s 16676 发 0 错 p95 83ms |

### 5.8 生产环境验收 ✅（2026-07-18，AWS m8g.xlarge 4C16G Graviton4，betogo.games，`results/prod-2026-07-18/`）

口径与 5.1-5.3 完全一致：k6 服务器本机 LOCAL=1、种子 500 用户、LT-1..50 每人 3000 局历史（15 万局/30 万注单）、LTD 下线 2400、限流旁路。差异：VUS 阶梯按 4 核加高档（P1 重接口至 80、P2 至 40、P3 至 150）；pages.js tasks 页已对齐新业务（+/spin/status）。

**P1 关键接口对比（rps 平台值，全程 0 错误）：**

| 接口 | 测试机 2C2G | 生产 4C16G | 倍数 |
|---|---|---|---|
| /bets | 111-120 | ~240 | 2.1× |
| /team/tree | 73 | ~210 | 2.9× |
| /rebate/progress | 450（#11 后） | ~500-550 | 1.2× |
| /ledger | 健康 | ~500 | — |
| /wallet/balances | 268 | ~900 | 3.4× |
| /slots/games | 261-376 | ~790 | 2-3× |
| /tasks | 208（#2 后） | 404 | 1.9× |
| /home/content | — | 2090 | — |

**P2 页面打开/秒对比（@VU20，0 错误）：**

| 页面 | 测试机 | 生产 | 页面 | 测试机 | 生产 |
|---|---|---|---|---|---|
| games | 261 | 396 | tasks | 24 | 88 |
| home | 134 | — | rebate | — | 230 |
| bets | 121 | 272 | vip | 22 | 49 |
| startup | 105 | — | team | 26 | 55 |

vip 页 @40 整页 p95 922ms 仍是全站最重页（一屏 5 聚合），量级可接受。

**P3 系统拐点**：吞吐平台 **~135 页面打开/s（≈575 req/s）**，测试机 60 打开/s → **系统容量 2.3×**。VUS 10→150 吞吐恒定、延迟线性涨（p95: 130ms→2004ms），err 全程 0；舒适区 p95<800ms 在 VUS≈60-100 之间。**重要发现：P3 饱和时 load 仅 2.0-2.6/4 核、内存动都不动（13.6G 空闲）——瓶颈不再是 CPU/内存，是 bff 单进程事件循环+连接池 10 的软上限**。下一档免费扩容抓手：bff 多实例（2 进程即可吃满 4 核）+ 池调大（优化#6），无需升配。

按 P3 测试机换算口径（60 打开/s ≈ 600 同时活跃）：生产当前配置支撑 **~1300-1400 同时活跃用户**；对外真实能力还要计入公网带宽/CDN（本测走本机，未含带宽墙）。

**事故与修复**：见优化#13（连接池死锁，生产 P2 实锤并当场修复复测通过）。tasks@40 之后至 wallet-history 的首轮数据为死锁期间假数据已剔除，tasks/rebate/vip/wallet-history/bets 五页为修复后重跑数据。
