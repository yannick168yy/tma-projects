...
| 注册 / 登录 | | | | — | — | 🔴阻塞：注册 id 生成缺陷（见优化#8），修复后再压 |
| 充值/提现落单 | | | | | | ⬜ |
