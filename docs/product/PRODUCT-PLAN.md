...
| TG 登录、统一钱包、首页直达游戏 | 自研游戏 / 平台自营游戏 |
| TG Wallet + 三方充提（法币+币） | Telegram **Stars** 支付（规避封禁与平台政策风险） |
...
| 通道 | 用途 | 注意 |
|------|------|------|
| **Telegram Wallet** | TG 生态内便捷支付 | 使用官方 **Wallet** 能力；**不使用 Stars** 承载充值，降低 Bot/Mini App 被封风险 |
...
| WL-03 | 流水类型 | DEPOSIT/WITHDRAW/ORDER/PAYOUT/RED_PACKET_*/ADJUST（竞彩下单/派彩） | P0 |
...
| PY-06 | 禁用 Stars | 产品与技术均不接入 Stars 充值 | P0 |
...
`DEPOSIT` | `WITHDRAW` | `ORDER`（竞彩下单）| `PAYOUT`（竞彩派彩）| `REFUND` | `RED_PACKET_TRIAL` | `RED_PACKET_REFERRAL_*` | `ADJUST` | `FEE`
...
```
有效流水 = sum(符合条件的竞彩下单金额) - 取消/无效竞彩记录
...
| 领域 | 策略 |
|------|------|
| **目标市场** | 菲律宾；需本地竞彩/支付法务评估 |
| **TG 支付** | 优先 **Telegram Wallet**；**禁止 Stars 用于充值** |
| **KYC** | 法币三方提现：基础 KYC 必做；资料最小必要留存 |
| **理性竞彩** | 单日充值上限、自我排除（后台配置，P1 可加强） |
