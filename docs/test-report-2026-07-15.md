...
| 编号 | 用例 | 状态 |
|---|---|---|
| TEAM-004 | 佣金提现（人工审核） | ⏳ 未测（需佣金钱包余额 + 走 team 审核队列） |
| TEAM-005 | 佣金提现风控规则（佣金激增/新下线占比/佣金>存款0.5/同IP下线） | ⏳ 未测（team-withdraw-review，规则与 user 侧同构） |
| AGENT-001/002 | 渠道代理（bg_agent_*，独立系统） | ⏳ 未测 |

> 收尾：所有测试数据（佣金/钱包/树/测试用户）已清理，BG-10001/10002 真实态完好。
