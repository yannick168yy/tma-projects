...
### P1-4 域名管理 + 证书巡检 · 3d ✅ 核心已完成 2026-09-03（`bc7c0947` / `dd48e5ac`）
...
**未完成**：**ACME 自动签发**。当前全程只读——巡检不签发、不改 nginx。
签发要改动线上配置，定为人工确认后单独触发。第一个自带域名的客户仍需手工签一次证书。
...
**未完成部分**：域名自动证书（P1-4）、win568 子代理自动注册、支付通道自动配置。
这三项都依赖外部系统，先手工配。
