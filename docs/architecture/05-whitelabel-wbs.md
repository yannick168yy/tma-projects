...
| P1 开站与平台后台 ✅ **已完成** | 能开出第二个站，定制化第一批交付 | 17 | ~46 人天 |
| P2 商务闭环 🟢 **11.5/12 完成** | 双资金模式 + 四种分成 + 账单结算 | 12 | ~38 人天 |
| P3 定制化深化与生态 🟢 **8/8 交付**（P3-8 按决策只拆缝） | 区块化、overlay、自助、风控联防 | 8 | ~40 人天 |
...
> 进度（2026-09-05）：P2-1~P2-7、P2-9~P2-12 已完成并在阿里云测试环境跑通全链路
> （挂方案 → 日切快照 → 试算 → 出账 → 客户确认 → 核销扣额度 → 逾期降级 → 对账）。
> P2-8 只剩「支付调用切租户凭据」，已确认等有真实客户自带通道时再接。

### P2-1 计费与账户表 · 2d ✅ 已完成 2026-09-05
`infra/database/platform/008_billing.sql`：`pf_billing_plan`、`pf_billing_rule`、
`pf_tenant_billing_plan`、`pf_billing_daily`、`pf_invoice`、`pf_invoice_item`、
`pf_tenant_account`、`pf_tenant_ledger`、`pf_manual_queue`；`pf_tenant_channel` 补通道费率。

- 分成方案与功能套餐（`pf_plan`）分表：常见组合是「旗舰版功能 + 低分成」，
  塞一张表必然出现互相牵连的改动
- 结算币种统一 USDT，折算率取**当日快照**而非结算时实时汇率 ——
  否则同一批流水账单早出晚出金额不同，客户永远算不平
- 种子三档方案：标准分成（GGR 30% + 充值佣金 1%）/ 进阶（GGR 25% + 月费 500 保底）/ 纯月费

### P2-2 计费引擎（四种规则可组合）· 5d ✅ 已完成 2026-09-05
`services/billing/billing-engine.ts`，纯函数无 IO，20 个用例锁口径。

- 四种规则任意组合叠加；分档支持 `flat`（落在哪档整体按该档）与 `progressive`（逐段累进），
  语义必须写死 —— 同一组档位两种算法能差出一倍
- `settle_mode`：`sum`（月费+分成）/ `max_of_fee`（月费是保底，取较高者）
- 充值佣金支持 `scope`（全部 / 仅平台代收 / 仅自带通道）：混用双模式时两种费率不同
- 月费不足整月按天折算：试用客户月中开站，直接收全月立刻吵起来
- 流水返点支持分场馆费率，缺省场馆回落总费率

### P2-3 GGR 口径实现 · 2d ✅ 已完成 2026-09-05
口径写死在引擎注释与账单明细里，四个参数逐租户可配：
`deductBonus` / `deductCommission` / `deductChannelFee` / `carryOver`。

```
GGR    = 有效投注 − 派彩
净收益 = GGR − (扣活动成本) − (扣团队佣金) − (扣通道手续费，仅模式 A)
分成   = max(0, 净收益 + 上期结转) × 费率
结转   = min(0, 净收益 + 上期结转)，carryOver=false 时当期归零
```

账单每一行都带 `detail`：投注、派彩、每项扣减、净收益、上期结转、分档过程全部展开。
线上实测（自营站 2026-09）：GGR 0.841 − 活动成本 15.6794 = 净收益 −14.8384 →
当期 GGR 分成 0、结转 −14.8384，充值佣金 12.0149 × 1% = 0.1201。

### P2-4 日切快照任务 · 2d ✅ 已完成 2026-09-05
`billing-daily.service.ts`，每天马尼拉 05:00（UTC 21:00）跑，回填前三天。

- 数据源是租户库已有的 `bi_daily_*`（core-node 的 BI 日聚合产物），不重扫原始注单：
  省一遍全表窗口扫描，更重要的是「客户后台看到的数字」与「平台拿来收钱的数字」同源
- 必须晚于 core-node 的 BI 日聚合（马尼拉 04:00 补算前两天），抢在前面会把没补完的数锁进账单
- 锁定（`locked_at`）后拒绝覆盖；重算接口返回 `{ written, skippedLocked }`
  —— `ON DUPLICATE KEY UPDATE` 值没变时也返回 `affectedRows=1`，直接拿它计数会把
  「因锁定没改」报成「已重算」
- 汇率缺失记 fx=0 而不是 1：1 会把 PHP 当 USDT 算，账单直接放大 50 倍；出账时缺汇率直接拒

### P2-5 账单生成 / 确认 / 核销 · 4d ✅ 已完成 2026-09-05
状态机 `draft → issued → confirmed → settled`，`disputed` 只能回到 `issued`。

- **争议不能跳过确认直接核销**：跳过就等于平台单方面认定金额，而这正是争议要解决的东西
- 人工调整必须填原因（进审计），且 `confirmed` 之后不允许再改金额
- 出账即锁定该周期快照；同租户同周期唯一键挡住重复生成（不做先删后建：删掉的是客户可能已看过的账单）
- 核销从额度账户扣划，**允许扣成负数**（账单该收的钱不因客户没钱而消失），
  扣完为负则进人工队列

### P2-6 租户额度账户 · 3d ✅ 已完成 2026-09-05
- 可动用额度 = 余额 + 授信，**押金不计入**（押金是违约金来源，不是运营资金）
- 流水表只 INSERT：记错了只能反向再记一笔，能改就不是事实依据
- `(租户, 业务类型, 业务单号)` 唯一键让重复核销、代付重试天然幂等
- 额度不足**不自动拒绝、不平台垫付** → `pf_manual_queue`

### P2-7 资金模式 A（平台统一代收代付）· 5d ✅ 已完成 2026-09-05
已完成：
- `220_settlement_mode.sql`：`bg_deposit_order` / `bg_withdraw_order` 加 `settlement_mode`，
  **建单时定、之后不改**（通道归属会变，回溯会让客户已确认的账单被重算出另一个数）
- 通道归属解析 + 300s 缓存；平台控制台可登记归属、费率、商户号
- 🔴 未登记通道默认 `platform`，依据是部署事实：支付凭据来自平台环境变量，
  钱进的是平台商户号。默认 `tenant` 会让平台少收自己垫付的手续费，
  并把平台代收的充值算成客户自收 —— 差异直接落在客户该付多少上
- 平台代收的手续费按费率进 GGR 扣减项

代付额度门禁（**作用范围已定：只约束已配押金/授信的非自营租户**）：
- 门禁放在 `approveWithdraw` —— 所有渠道放款的唯一入口，四条充提路径不用各改一遍
- 额度不足 → 写人工队列 + 抛错，订单留在 `pending`：不自动拒绝玩家、不平台垫付
- 🔴 自营站与未配额度的租户完全放行。反过来（一律纳管）会让上线瞬间所有提现全部转人工，
  因为自营站的额度账户里没有对应真实资金的余额。**代价是忘记配额度就等于这家没门禁** ——
  宁可漏一家，也不能让一个配置遗漏卡住所有站的提现
- 放款成功即扣额度（ref=订单号，靠流水唯一键幂等）；支付商侧最终失败的由日切
  `reverseFailedPayouts` 反向冲回 —— 平台没垫出的钱不能长期占着客户额度，
  否则表现为「莫名其妙被停提现」
- 汇率取不到时报错而非按 0 折算（按 0 折算等于门禁形同放行）

### P2-8 资金模式 B（租户自带通道）· 3d ⚠️ 凭据存储已完成，支付调用待决策
已完成：
- `lib/secret-box.ts`：AES-256-GCM，密钥取 `TENANT_CREDENTIAL_KEY`；
  **没配密钥时拒绝加密而不是明文落库**（通道密钥泄露等于客户的钱能被直接提走）
- 后台只显掩码（`mchN********7890`）；解不开时显式提示「可能换过密钥」，不装作未配置
- `assertChannelUsable`：登记为租户自带却没配凭据时拒绝建单 ——
  静默回落平台凭据会让客户玩家的钱进平台商户号、账单又按自收算，两头都错

**剩余部分已确认延后**（2026-09-05 用户决定）：把 unispay / yfpay 的调用切到按租户凭据、
以及回调按 `merchant_no` 反查租户，等有真实客户自带通道时再逐家接 ——
各支付商凭据字段不同，需按客户实际用的支付商定 schema，没有真实通道时接了也无法验证。
在此之前所有站点实际都跑模式 A（凭据来自平台环境变量），与 P2-7 的默认判定一致。

### P2-9 混用模式对账 · 3d ✅ 已完成 2026-09-05
按 `(通道, settlement_mode)` 分组统计，混用租户显式标记，可展开看每通道笔数与手续费。
数字取自**已锁定的计费快照**而不是重扫订单：对账要对的就是「出账用的那份数」。

### P2-10 欠费三级降级 · 2d ✅ 已完成 2026-09-05
- 逾期天数分级（默认 3/7/14 天 → 停提现/停充值/停站），宽限期走环境变量；
  按逾期天数而不是余额是否为负 —— 天数是合同里认过的口径，降级时说得清
- 每级之前先进人工队列（`warnDays`，默认逾期 1 天），留商务介入窗口
- 只往更严格方向自动走；恢复由核销触发（`restoreIfCleared`），
  人工停站（如合规问题）不会被一次付款自动解开
- 生效点是新增的 `tenant-gate` 中间件。**踩过的坑**：第一版路径清单写的是
  `/api/v1/withdraw`，真实挂载路径是 `/api/v1/withdrawals`，不存在的路径照样返回 403，
  看起来「生效了」而实际一个都没挡住。现在 5 个用例锁的是真实路径
- 豁免 `/admin`（客户要能进后台看为什么停了）、`/platform`、`/webhooks`；
  只挡创建不挡查询；支付回调落在 core-node，走不到这个中间件

### P2-11 平台总览 BI · 4d ✅ 已完成 2026-09-05
`010_bi_daily.sql` + `platform-bi.service.ts`，平台控制台首页。

- **不做实时跨库 UNION**：租户数上去后一次总览会同时压所有租户库，任何一个慢就整页转圈
- 与 `pf_billing_daily` 分表：计费快照出账后必须锁定，BI 要能随时重算，
  一张表兼顾两个相反要求必有一个被牺牲
- 每 30 分钟刷当天与昨天，日切那轮回填前三天
- 汇率缺失的币种不折算但记 `skipped_rows`：总额偏小时能一眼看出是生意差还是汇率没抓到

### P2-12 租户后台账单页 · 3d ✅ 已完成 2026-09-05
`/admin/billing/*` + 后台「系统设置 → 平台账单」（限 super_admin / finance）。

- 🔴 一律以 `currentTenant().id` 为准，不接受前端传 `tenantId`
- 客户只能确认或提争议：金额是按签约规则算的，客户改不了自己该付多少，
  但每一项算法必须摊开，否则确认按钮点不下去
- 导出 CSV 而非 PDF：客户拿去对账用的是表格，PDF 还得手工再抄一遍

### P2-X 平台控制台信息架构拆分 · ✅ 已完成 2026-09-05
`TenantDetail.tsx`（689 行、一屏 8 张卡片）拆成 `/tenants/:id/{overview,plan,brand,i18n,domains,channels,billing}`
七个子路由；顶层新增平台总览、套餐管理、分成方案、账单、额度与队列、资金模式对账。
套餐可覆盖范围（P1-14）从租户详情移到套餐管理页 —— 区间是套餐级配置，
放在某一家的详情页里会让人误判影响面。
...
### P3-1 首页区块注册表（L2 核心）· 6d ✅ 已完成 2026-09-05
P1-13 已把顺序/显示/参数做成服务端下发，这次拆的是区块本身（HomeContent 837 → 526 行）。

- `components/home/gameSections.tsx`：12 个游戏块原本是 12 段几乎一样的 JSX
  （图标+标题+查看全部+网格），差异只有四项，改成一张声明表。加专区 = 加一行
- 行为差异显式声明而不是借重构悄悄改前台：`marginTop`（popular 用 mt-5）、
  `hideWhileLoading`（highRebate 加载中不占位）、`enabled`（recommended 只在
  最近在玩占了上方那行时出现）
- `BettingTable.tsx` / `BannerCarousel.tsx` 各自管自己的状态搬出去（榜单三份懒加载、
  轮播手势与自动播）；`primitives.tsx` 把 SectionHeader/BigGrid/SmallRow 改成纯组件
  —— 原来闭包在 HomeContent 里，区块拆出去带不动
- 兜底顺序独立成 `blockOrder.ts` + 开发期断言「区块表与顺序一一对应」：
  加了块忘排顺序只表现为某块偶尔不出现，测试环境极难复现
- 后台「首页布局」改原生 HTML5 拖拽（不引 dnd 库），↑↓ 保留做精调

### P3-2 底部导航与页面开关可配 · 2d ✅ 已完成 2026-09-05
`221_bottom_nav.sql` + `/site/config` 下发 + 后台「系统设置 → 底部导航」。

- 顺序、显示、图标、跳转目标可配；**槽位数量固定 5 个不可增删** ——
  每个槽位背后是一个已存在的页面组件，凭空多一个 id 不会有页面跟着长出来
- 图标与跳转目标走白名单：写错图标名 = 底栏少一个图标，写错路径 = 点了没反应
  （parseAppRoute 返回 null 会 replace 回首页），两者在后台保存时都发现不了
- 两条硬约束：首页槽位不可隐藏、至少留两个入口
- 前端点击按 targetPath 而不是槽位 id，三圈原来的「特判成浮层」变成通用行为

### P3-3 活动模板市场 · 6d ✅ 已完成 2026-09-06
`012_promo_template.sql` + 平台控制台「活动模板」+ 租户后台自助套用。

- 🔴 **模板 = 一套验证过的活动参数，不是通用活动 DSL**。现有 8 类活动的领取条件、
  流水锁、结算时机各不相同（首充一次性、复充有窗口与冷却、负盈利按日结算），
  硬塞进一套 trigger/condition/reward 模型要么表达不了，要么算出来的钱与现有实现
  有细微差别 —— 活动算错钱是直接资损。真实痛点也不是造新玩法，是「照 X 站那套来」，
  所以路径是：调好一家 → 导出成模板 → 套给后面的
- 模板只存补丁（覆盖哪几个区块存哪几个），套用按区块级替换：
  「只调首充档位」的模板不会顺手冲掉人家的弹窗配置。深合并档位数组的语义没人说得清
- 校验从后台路由抽成 `validatePromoConfig` + `mergePromoConfig`，套用与后台改配置
  共用同一份 —— 两份漂移的后果是「后台改不进去的值，套模板能进去」，资损口子
- 套用前存快照进 `pf_promo_template_apply`：客户问「活动怎么变了」要拿得出改动前的样子
- 租户后台可自助套用（P3-5 的一部分）：只能套到自己身上（tenantId 取上下文），
  模板内容不下发给租户 —— 那是别家调出来的参数
- 踩过两个坑：抽出纯函数后 `vi.mock` 缺导出导致 8 个用例失败（改 importOriginal 部分 mock）；
  按 `markets[0]` 过滤让多市场租户一个模板都看不到（改成全部市场 IN）
...
### P3-5 租户自助能力 · 6d ✅ 已完成 2026-09-06
`013_self_service.sql` + 租户后台「系统设置 → 自助配置」（支付通道 / App 出包 / 开放 API）。

- 🔴 所有自助接口都以 `currentTenant()` 为准，**接口上没有 tenantId 参数** ——
  带 tenantId 只要漏一次校验就是跨租户越权（P1-7 踩过）
- 自助边界写在服务层，不靠前端隐藏：
  · 只能改 `owner='tenant'` 的通道凭据 —— 平台代收通道用的是平台商户号，
    客户改了等于把收款账号换到别处，那是资金被劫持不是配置错误
  · 费率与通道启用状态不给改（商务合同定的）
  · 包名与签名引用名以库里为准、忽略入参：包名换了等于换一个 App，老用户收不到更新
- 出包做成「申请」而不是按钮：签名密钥只在出包机上，放到服务器就等于别人能给
  客户已发布的 App 推更新。`(tenant, market, status)` 唯一键挡住连点
- 活动配置自助套用模板见 P3-3
- 平台侧 `/platform/app-builds` 待办：完成必须回填产物地址，驳回必须写原因

### P3-7 开放 API · 5d ✅ 已完成 2026-09-06
`014_open_api.sql` + `/api/open/v1` + `docs/product/OPEN-API.md`。

- 把「后台再加一个报表 / 导出某个字段」的无底洞需求，从「改我们的代码」
  变成「客户写他自己的脚本」
- 🔴 只存 sha256 摘要；完整 key 只在创建那一次返回 —— 能随时查看意味着
  任何一个能进后台的人都能拿走全部 key
- 🔴 鉴权中间件用 `runWithTenant` 包住后续处理：开放 API 的 handler 都直接
  `getMysqlPool()`，租户上下文是唯一隔离手段，忘了包等于所有 key 都能读自营库
- 挂 `/api/open/v1` 而非 `/api/v1/open`：两个 v1 不是一回事
- 限流按 key 不按 IP（客户服务器换 IP 正常，一把 key 打爆连接池会影响前台玩家）；
  响应头带 `X-RateLimit-*`，超限 429 + `Retry-After`
- 失败原因分类返回（缺哪个 scope / IP 没加白 / 已吊销 / 已过期），
  但不区分「摘要不匹配」与「前缀不存在」—— 那会变成枚举前缀的信道
- v1 全部只读，且刻意不下发手机号邮箱与收款账号：后台能看且有审计，
  API 是会落进客户自己库里的，少一层扩散面
- 时间窗最长 92 天、pageSize 上限 200
- 线上实测：30/分钟限流精确命中（30×200 后 429）、缺 scope 403、吊销后立即 401

### P3-6 跨租户风控联防 · 5d ✅ 已完成 2026-09-05
`011_risk_federation.sql`：`pf_risk_blacklist` / `pf_risk_identity` / `pf_risk_hit`。

- 🔴 **只存 HMAC 摘要 + 掩码，不存明文，也不提供反查**。平台库里躺着几十家客户的
  玩家手机号与收款账号，一次拖库就是所有客户一起出事；联防只需要判断
  「同一个值是否出现在多家」，摘要就够。能反查等于平台留了一份可导出的客户玩家名册
- 🔴 没配 `RISK_FEDERATION_PEPPER` 时整个功能关闭，不降级成裸 sha256：
  手机号只有 10 位数字，不加盐能被穷举反查，等于明文入库
- 归一化吃掉写法差异（手机号国码、卡号空格、IPv4 映射前缀）—— 不归一化不是「效果差点」
  而是**静默不生效**；不同类型加类型前缀，避免设备 abc 与手机号 abc 撞在一起
- 判定叠加取更严：平台名单是别家客户报上来的团伙证据，本站没吃过亏不代表这人是好人；
  反过来本站已判 deny 的，平台说 watch 也不放行
- 命中同时写租户自己的命中日志：客户看不到原因只会来问平台
- 抽数四类（注册设备 / 登录手机号 / 提现收款账号 / 证件号），日频；
  **IP 不参与自动抽数** —— 家用宽带动态 IP 会把同城玩家全串成一伙，噪声大到没法用，
  但保留 ip 类型供手工拉黑机房出口
- 🔴 顺带修一个真空转：`/payment/withdraw/create`（最常用的法币提现）此前完全没走风控，
  `bg_risk_policy` 的 withdraw 规则在这条路径上一直是空的
- 平台控制台「风控联防」：名单 CRUD + 撞库榜（≥2/3/5 家）+ 一键拉黑
- 线上验证：抽出 26 条摘要（4 设备 / 21 手机 / 1 收款账号），构造同设备跨两家 →
  撞库榜正确列出 `['betogo','demo1']`，一键拉黑后 60 秒内全平台生效；库里只有摘要

### P3-8 多聚合商接入抽象 · 5d 🟡 只拆缝，不做抽象（2026-09-06 用户确认）
`lib/aggregators.ts`（bff-node / core-node 各一份）+ `docs/architecture/06-aggregator-integration.md`。

**没有做 provider 接口层，这是刻意的**：只有一个实现时设计的抽象会把这一家的假设焊进去
（幂等键形状、转账钱包 vs 无缝钱包、结算时机、作废方式、密钥轮换），第二家来了照样不合；
而改造面落在 `win568-wallet.service.ts`（891 行注单与钱包扣加）—— 在只有一家时重构它，
收益是「看起来可扩展」，风险是「钱算错」。

这次做的是把缝留出来：
- `aggregator_id` 取值集中到常量（两个服务各一份，值必须一致 ——
  它落在 `bg_bet_order.aggregator_id` 列上，两边不一致会静默查不到数据）
- 两服务 28 处 `'568win'` 字面量全部改为引用常量：
  **第二家接入时 grep `DEFAULT_AGGREGATOR` 就是「需要按聚合商分流」的完整清单**
- SQL 用模板插值而非新增占位符：那几条查询参数数组很长（VIP 负盈利十几个参数），
  重新编号占位符是最容易埋 bug 的改法；插的是联合类型编译期常量，无注入面。
  构建产物做了反引号奇偶校验，确认 0 处插值落在普通字符串里
- `06-aggregator-integration.md` 是从现有代码描出来的实况清单（六块能力 + 接入检查清单），
  不是凭空的接口设计稿

**抽象的触发条件**：第二家的接口文档在手，两家差异摆在一起，才知道抽在哪一层。
...
- ✅ **M2（P1 完成，2026-09-05）：能对外开出第一个包网客户站**

  > **上线前还有两个卡口，都不是代码：**
  > 1. **生产环境仍是 `TENANT_RESOLVE_STRICT=false`**（只有测试环境切了 true，见 P1-0e）。
  >    切之前必须先把 nginx 上服务的域名全部登记进平台库 —— 测试环境实测漏了
  >    `admin.188facai.com` 与 `a001.188facai.com`，直接切会打断线上入口
  > 2. **`PLATFORM_CREDENTIAL_KEY` 还没配**（P1-5）。不配就存不了任何第三方凭据，
  >    平台后台的子代理与通道表单会直接报错
### 🔴 生产开新站前必须先解决的三件（2026-09-07 核查，决定「开站时再处理」）

生产至今只有自营站一个租户，**一键开站在生产从没跑过**。这三项不解决，
第一次点「开站」会直接失败或埋雷。测试环境不受影响（那边已开出 demo1，123 张表）。

1. **bff 容器没挂载 `infra` 目录** —— 卡死项。
   开站建表要读 `/app/infra/database/betogo/schema_baseline.sql`，生产容器里没有这个路径，
   会在「建表」步抛 `读不到结构基线`。
   属于历史漂移：`recreate-bff-node.sh` 里有 `-v "$DIR/infra":/app/infra:ro`，
   生产容器是在那行加进来之前建的。
   修法：重建两个 bff 容器补上该挂载（宿主上文件是有的）。

2. **`TENANT_RESOLVE_STRICT` 生产仍是 `false`** —— 数据串号风险。
   客户域名漏登记时，玩家会落到**自营站的库**上。
   技术障碍已清（`8be59f53` 让 `/api/v1/platform/` 不走租户解析，切 true 不再会把平台后台
   自己打成 404）。切之前仍要复核 `*.betogo.games` 泛解析：切后未登记子域会 404 而非兜底。

3. **平台域名未登记进 `pf_tenant_domain`** —— 已由 `8be59f53` 从代码侧绕开，
   但若将来给平台后台加非 `/api/v1/platform/` 的接口，这个坑会复活。

> 已在 2026-09-07 补齐：生产缺失的 `issue-tenant-certs.sh` 与 `nginx-tenant-site.conf.tpl`
> 已同步；`daily-backup.sh` 已改为按 `pf_tenant` 逐库备份（含平台库）并做过恢复演练；
> `tma-web-platform` 已纳入心跳告警。
