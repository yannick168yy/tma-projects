...
| P2 商务闭环 🟡 **10/12 完成** | 双资金模式 + 四种分成 + 账单结算 | 12 | ~38 人天 |
...
> 进度（2026-09-05）：P2-1~P2-6、P2-9~P2-12 已完成并在阿里云测试环境跑通全链路；
> P2-7/P2-8 的记账与配置面已完成，**真正切换资金路由的两处待决策**（见 P2-7 / P2-8 末尾）。
...
### P2-7 资金模式 A（平台统一代收代付）· 5d ⚠️ 记账面已完成，资金路由待决策
已完成：
...
**待决策**：代付放款前的额度门禁作用范围。自营站与未配额度的租户若一并纳管，
所有提现会立刻全部转人工（自营站额度账户里只有测试用的 99.9 USDT），
这是会打断线上业务的改动，需要明确谁受约束、门禁不通过时订单停在哪个状态。
...
**待决策**：把 unispay / yfpay 的调用切到按租户凭据（凭据字段各家不同，需逐家定 schema），
以及回调路由如何按 `merchant_no` 反查租户。这一步真正改变钱的流向，需要先确认
有哪家客户要自带通道、用的是哪个支付商。
