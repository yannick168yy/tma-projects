...
1. **点控 RTP 后台**（档位限 50–97）
2. **对账 cron**（`get_game_history_list`）
3. **包网租户化**：bff 目前调根路径 `/internal/wxgame/...`，日志已警告
   「回调来源无法归属租户，已回落自营站」。租户段路由已注册，接包网客户前 bff 要改用
   `/t/<code>/internal/wxgame/...`（568win 同样存在此问题，非本次引入）
4. 图片落 OSS + WebP 压缩
5. 正式环境 25 家厂商的图源复测
...
- [ ] 点控 RTP 后台（档位限 50–97）
- [ ] 对账 cron（`get_game_history_list`）
- [ ] 包网租户化：bff 改调 `/t/<code>/internal/wxgame/...`
- [ ] 图片落 OSS + WebP 压缩；正式环境 25 家厂商图源复测
- [ ] 一轮完整回归：下注 → 派彩 → 撤销 → 洗码 → 负盈利返水 → 团队佣金
