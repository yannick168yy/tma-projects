...
> 4. 当前 SP 利用率仅 **60.6%**，剩余约 $24.17/月仍在浪费。加一台
>    **m8g.medium（1C4G，消耗 $0.0255/h）**可把利用率提到 **91%**，
>    该实例费同样为 $0，只需付其 EBS（30GB 约 $2.9/月）。
>    **但现在先不要开** —— 阿里云测试机已预付到 2027-06-12，此刻开只是多花钱养一台
>    没用的机器。执行时机见文末《2027-06-12 决策点》。
>
> ### 仍未确认
>
...
# 决策点：2027-06-12（阿里云测试机到期）

**阿里云测试机（`47.84.34.139`）于 2026-06-12 购买一年，2027-06-12 到期。
到期时不要直接续费**，先评估用 AWS `m8g.medium` 接管。

## 为什么是这个时间点

SP 承诺额 $0.084/h 是按 m8g.xlarge 的规模签的，当前跑 m8g.large 只消耗
$0.0509/h，**每月有 $24.17 的额度白付，且无法退订或缩减（锁到 2029-07）**。
补一台 m8g.medium（消耗 $0.0255/h）可把利用率从 60.6% 拉到 91%，
**该实例的计算费为 $0**，只需付其 EBS（30GB 约 $2.9/月）。

但这笔账只有在阿里云到期那一刻才划算：

| 时机 | 净效果 |
|---|---|
| 现在就开 | 阿里云钱已预付，纯多花 $2.9/月养一台用不上的机器 |
| **2027-06-12 到期时开** | 省下阿里云续费 + 白得 1C4G + SP 利用率 60.6% → 91% |

## 到期前要做的事

1. 确认 SP 仍在有效期内（2029-07 到期，届时还剩约 2 年）—— 若 SP 已到期则此方案不成立
2. 评估 1C4G 是否够跑测试环境（生产 2C8G 实测 CPU 仅用 1.5%，测试环境负载更低）
3. 迁移部署配置：`CLAUDE.md` 里的测试环境部署参数需改 `DEPLOY_HOST` 与密钥路径

## 另一项独立优化（与上面无关）

EBS 根卷 100GB 实际仅用 36GB，缩到 50GB 可省 $4.8/月。但 EBS **不能直接缩容**，
需建新卷 → 迁移数据 → 换根卷，有停机和风险。单为 $4.8/月不值得，
建议搭车在其他必须停机的维护窗口里顺带做掉。
