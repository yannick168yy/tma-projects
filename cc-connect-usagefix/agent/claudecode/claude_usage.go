...
func (a *Agent) runClaudeUsageProbe(ctx context.Context) (string, error) {
	...
	for {
		...
		case <-ticker.C:
			...
			if usageScreen == "" && time.Since(lastChange) >= claudeUsageAPIBillingStableFor &&
...
func promptActionForScreen(screen string) string {
	...
	if claudeUsageLooseContains(lower, "quick safety check") ||
